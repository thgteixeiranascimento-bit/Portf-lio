# cashtag-entity-ux Specification

## Purpose
Define the GUI cashtag entity layer: composer autocomplete for `$`-prefixed ticker entry, chat-text entity chips, server-projected known symbols, and chip popovers that reuse cached market-state context without adding new provider fetch paths.
